from .osu_file_parser import parser
...
def calculate(file_path, mod, lambda_2, lambda_4, w_0, w_1, p_1, w_2, p_0):
        ...
        if not note_seq:
            raise ValueError(f"{file_path.split('\\')[-1].rstrip('.osu')} has no notes.")
        ...
        if len(note_seq_by_column) != K:
            raise ValueError(f"{file_path.split('\\')[-1].rstrip('.osu')} has one or more empty columns.")
